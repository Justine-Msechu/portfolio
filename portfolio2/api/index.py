...
def index():
    try:
        ...
        return f"<h2>Error</h2><pre>{e}</pre><p>Make sure DATABASE_URL is set in your .env file.</p>", 500
...
def update_profile():
    ...
    """, (data.get('name'), data.get('title'), data.get('bio'),
          data.get('email'), data.get('github'), data.get('linkedin'), data.get('location')))
    conn.commit(); cur.close(); conn.close()
